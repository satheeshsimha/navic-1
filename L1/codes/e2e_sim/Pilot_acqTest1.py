...
def genNavicCaTable_Data(samplingFreq, satId):
   # prnIdMax = 64
    codeLength = 10230
...
def genNavicCaTable_Pilot(samplingFreq, satId):
    #prnIdMax = 64
    codeLength = 10230
    codeFreqBasis = 1.023e6
    samplingPeriod = 1/samplingFreq
...
def genNavicCaTable_Pilot_Overlay(samplingFreq, satId):
  #  prnIdMax = 64
    codeLength = 1800
    codeFreqBasis = 1000
    samplingPeriod = 1/samplingFreq
...
class NavicTracker:
    ...
    def setupImpl(self):
        ...
        t = np.arange(0,1,self.ChipRate*10)
        subCarrier = np.sign(np.sin(2*np.pi*(self.subcarrierFrequency*t)))
        
        code = (1 - 2 * genNavicCaCode_Data(self.PRNID).astype(float))*subCarrier # added subcarrier multipication
        ...
        numSamplesPerCodeBlock = self.SampleRate * 10e-3 # As each code block is of 10e-3 seconds
...
codeTable_data = genNavicCaTable_Data(sampleRate,satId)
codeTableSampCnt_data = len(codeTable_data)


codeTable_pilot = genNavicCaTable_Pilot(sampleRate,satId)
codeTableSampCnt_pilot = len(codeTable_pilot)


codeTable_pilot_overlay = genNavicCaTable_Pilot_Overlay(sampleRate,satId)
