...
def navic_pcps_acquisition(x, prnSeq_pilot, prnSeq_pilot_overlay,fs, fSearch, threshold=0):
...
class NavicTracker:
    def __init__(self, prnId, pilotCodeLength, pilotOverlayCodeLength, symbolRate):
    ...
    def updatePromptCode(self):
        ...
        code = pilotSig*subCarrier # added subcarrier multipication
    ...
    def stepImpl(self, u):
        ...
        iq_e = iqsig * np.roll(self.pPromptCode, -1*numSamplesPerHalfChip) # Early
        iq_ve = iqsig * np.roll(self.pPromptCode, -1*numSamplesPerThreeQuarterChip)# Very Early
        iq_p = iqsig * self.pPromptCode # Prompt
        iq_l = iqsig * np.roll(self.pPromptCode, numSamplesPerHalfChip) # Late
        iq_vl = iqsig * np.roll(self.pPromptCode, numSamplesPerThreeQuarterChip) # Very Late
        ...
        millisecdata = iq_p.reshape((int(self.PLLIntegrationTime/10), -1)).T # Each column contains ten millisecond of data
        ...
        if self.DisablePLL:
            ...
            #pherr = np.arctan(np.imag(integpval)/np.real(integpval))
...
for istep in range(numSteps):
    ...
    #Baseband modulation
    iqsig = modulator.Modulate(navdata,codeTable_data[:,np.arange(numChannel)],codeTable_pilot[:,np.arange(numChannel)],codeTable_pilot_overlay[:,np.arange(numChannel)])
    ...
    if istep == 0:
        ...
        for prnId in range(numChannel):
            status, codePhase, doppler = navic_pcps_acquisition(
                                            ...
                                            codeTable_pilot_overlay[np.arange(0, samplePerStep)%codeTableSampCnt_pilot_overlay, prnId],
                                            sampleRate ,fSearch
                                        )   
            delaySamp = codePhase
...
# If a satellite is visible, initialize tracking loop
            if(status == True):
                satVis += 1 
                tracker.append(NavicTracker(satId[prnId], pilotCodeLength, pilotOverlayCodeLength, symbolRate))
