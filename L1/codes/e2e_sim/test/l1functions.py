...
class NavicL1sModulator():
    ...
    def Modulate(self, x, codeTable1,codeTable2,codeTable3):
        if_frequency = 5.42e6  # IF frequency
        ...
        iqsig = (alpha*(BocPilotSig1)-beta* (BocPilotsig6 ))*np.cos(2*np.pi*if_frequency*t)[:, None] - (gamma*BocDataSig1+eeta*interplexSig)*np.sin(2*np.pi*if_frequency*t)[:, None]  # Document formula
    ...
    for i in range(0,N):
        ...
        for  a in range(0,non_coherent_integ):
            ...
            for b in range(0, coherent_integ):
                ...
                x_iq = x[start:end]*np.exp(-1j*2*np.pi*(if_frequency+fSearch[i])*t)
...
class NavicTracker:
    ...
    def updatePromptCode(self):
       ...
       # codeNumSample_pilot = self.codeTable_pilot.shape[0]
        ...
        # Subcarrier generation for BOC
        t = np.arange(round(self.SampleRate* self.PLLIntegrationTime*1e-3))/(round(self.SampleRate))
    ...
    #     #[localbuf, buf] = np.split(np.append(self.pBuffer, np.zeros(len(u))), [round(self.SampleRate*integtime)]) 
        ...
        # tau = 0
        # K = u.shape[0]
        # t = np.arange(self.pNumIntegSamples)/self.SampleRate
        # prnSeq_pilot_FFT = np.conjugate(np.fft.fft(self.pPromptCode_pilot))
        # for i in range(-200, 200, 20):
        #         p2n = 0
        #         Rxd = np.zeros(K)
        #         fc = self.CenterFrequency + self.InitialDopplerShift - self.pFLLNCOOut + i
        #         x_iq_p = u*np.exp(-1j*2*np.pi*fc*t)    
        #         XFFT = np.fft.fft(x_iq_p)
        #         YFFT = XFFT = np.fft.fft(x_iq_p)* prnSeq_pilot_FFT
        #         non_coherent_prod_p =  (1/K)*np.fft.ifft(YFFT)
        #         Rxd = Rxd + np.abs(non_coherent_prod_p)**2
        #         maxIndex = np.argmax(Rxd)
        #         max_val = Rxd[maxIndex]
        #         dummy = np.copy(Rxd)
        #         #print("f=", fSearch[i], "maxIndex=", maxIndex, "max val=", max_val)
        #         # plt.plot(Rxd)
        #         # plt.xlabel('time') ; plt.ylabel('RX')
        #         # plt.show()
        #         #print("max index=",maxIndex, "max val=", max_val)
        #         #Compute Peak to noise ratio
        #         for ind in range(-2,2,1):
        #             index = (maxIndex + ind+ K)%K # Make values 0 around one chip duration on either side of max value
        #             Rxd[index] = 0
        ...
        #             max_rxd = dummy
        #             # max_rxd = dummy
        #             # maxIndex = np.argmax(max_rxd)
        #             # max_val = max_rxd[maxIndex]
        #             # print("Inside if f=", fSearch[i], "maxIndex=", maxIndex, "max val=", max_val)
        ...
        #fft_axis = np.linspace(-0.5 * F, 0.5 * F, fft_len)
        #plot_val = iq_p_pilot * W
        #plot_val = signal.lfilter(b,a,iq_p_pilot)
        
        #plt.plot(fft_axis, np.abs(np.fft.fftshift(np.fft.fft(plot_val))))
        #plt.plot(fft_axis,np.abs(np.fft.fftshift(np.fft.fft(iq_p_pilot))))
       ...
       # print("data_prompt",np.abs(integpval_data))
        ...
        #fqyerr_pilot = np.arctan2(np.imag(phasor_pilot), np.real(phasor_pilot) )/(2*np.pi*integtime)
        ...
        #fqyerr_data = np.arctan2(np.imag(phasor_data),np.real(phasor_data))/(2*np.pi*integtime)
