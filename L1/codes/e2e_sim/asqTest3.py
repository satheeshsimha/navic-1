...
def navic_pcps_acquisition(x, prnSeq_data, fs, fSearch, threshold=0):
    ...
    sig = (1-2*prnSeq_data)*subCarrier
    
    #########   Plot Data  ############
    (f,S) = scipy.signal.periodogram(sig, fs, scaling='density')
    plt.subplot(3,1,3)
    plt.semilogy(f,S)
    plt.ylim([1e-12,1e-4])
    #plt.psd(iqsig, int(sampleRate/1000), 100) ; 
    plt.title('psd, subcarrier');
...
#########   Plot Data  ############
(f,S) = scipy.signal.periodogram(waveform, sampleRate, scaling='density')
plt.subplot(3,1,2)
plt.semilogy(f,S)
plt.ylim([1e-12,1e-4])
#plt.psd(iqsig, int(sampleRate/1000), 100) ; 
plt.title('psd, waveform');
plt.xlabel('frequency') ; plt.ylabel('PSD')
#plt.show()
##########  End of Plot Data  ############
