...
def navic_pcps_acquisition(x, prnSeq_pilot, prnSeq_data, fs, fSearch,coherent_integ, non_coherent_integ, threshold=3):
    ...
    prnSeq_data_FFT = np.conjugate(np.fft.fft(dataSig*subCarrier))
   ...
   # ts = 1/fs
    ...
    for i in range(0,N):
        non_coherent_prod_p = np.zeros(K, dtype=np.complex_)
        Rxd_p = np.zeros(K)
        non_coherent_prod_d = np.zeros(K, dtype=np.complex_)
        Rxd_d = np.zeros(K)
        for  a in range(0,non_coherent_integ):
            coherent_prod_p = np.zeros(K, dtype = np.complex_)
            coherent_prod_d = np.zeros(K, dtype = np.complex_)
            for b in range(0, coherent_integ):
                start_p = b*K
                end_p = (b*K)+(K)
                x_iq_p = x[start_p:end_p]*np.exp(-1j*2*np.pi*fSearch[i]*t)
                XFFT_p = np.fft.fft(x_iq_p)
                YFFT_p = XFFT_p*prnSeq_pilot_FFT
                coherent_prod_p = coherent_prod_p+YFFT_p
                # start_d = b*K
                # end_d = (b*K)+(K)
                # x_iq_d = x[start_d:end_d]*np.exp(-1j*2*np.pi*fSearch[i]*t)
                # XFFT_d = np.fft.fft(x_iq_d)
                # YFFT_d = XFFT_d*prnSeq_data_FFT
                # coherent_prod_d = coherent_prod_d+YFFT_d
                                
            non_coherent_prod_p =  (1/K)*np.fft.ifft(coherent_prod_p)
            Rxd_p = Rxd_p + np.abs(non_coherent_prod_p)
            # non_coherent_prod_d =  (1/K)*np.fft.ifft(coherent_prod_d)
            # Rxd_d = Rxd_d + np.abs(non_coherent_prod_d)
