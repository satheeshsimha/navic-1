...
#initial condition of register G2 taken from NavIC L1 ICD
SV_L5 = {
   1: '1110100111',
   2: '0000100110',
   3: '1000110100',
...
#PRN code generation for NavIC L1 constellation
...
def genNavicCaCode_Data(sv):
    ...
    :param int sv: satellite code (1-64 for NavIC L1)
...
def genNavicCaTable_Data(samplingFreq, codeLength, codeFreqBasis, satId ):
    ...
    :param  codeLength: length of Data PRN Code
    :param codeFreqBasis : chip rate (1.023e6)
    :param satId : Satellite Id (1-64)
    :returns array of upsampled Data PRN Codes
    """
...
def genNavicCaTable_Pilot(samplingFreq,codeLength, codeFreqBasis, satId):
    """
    :param  samplingFreq: Sampling Frequency
    :param  codeLength: length of Pilot PRN Code
    :param codeFreqBasis : chip rate (1.023e6)
    :param satId : Satellite Id (1-64)
    :returns array of upsampled Pilot PRN Codes
    """
...
def genNavicCaTable_Pilot_Overlay(samplingFreq, codeLength, codeFreqBasis, satId):
    """
    :param  samplingFreq: Sampling Frequency
    :param  codeLength: length of Pilot Overlay PRN Code
    :param codeFreqBasis : chip rate (1.023e6)
    :param satId : Satellite Id (1-64)
    :returns array of upsampled Pilot Overlay PRN Codes
    """
...
#This class takes care of modulating Data with Data PRN code and subcarriers & Pilot overlay code with Pilot PRN code and subcarriers
class NavicL1sModulator():
    def __init__(self, fs):
        ...
        self.m1 = 1; self.n = 1
        self.m2 = 6
        fsc1 = self.m1*1.023e6  # subcarrier 1.023e6
        epsilon1 = fsc1*1/(100*self.sampleRate)
        
        fsc2 = self.m2*1.023e6 # subcarrier 6*1.023e6
        epsilon2 = fsc2*1/(100*self.sampleRate)
        
        
    # columns of x have samples
    # columns of codeTable have sampled PRN sequences for given satellites
    def Modulate(self, x, codeTable1,codeTable2,codeTable3):
        """
        :param  x: upsampled NAV Data bits
        :param  codeTable1: upsampled Pilot PRN Code
        :param codeTable2 : upsampled Data PRN Code
        :param codeTable3 : upsampled Pilot OVerlay PRN Code
        :returns array of composite modulated signal elements without carrier
        """
        ...
        # PRN sequence of of pilot XORed with Pilot Overlay code
        PilotCode = (codeTable_p+codeTable3)%2
        PilotSig = 1-2*PilotCode #Convert to  BPSK
        
        # Data bits XORed with upsampled PRN codes and converted to BPSK
    ...
    #Generates subcarrier signal for a given value of n
    def __GenBocSubCarrier(self, N, m):
        '''
        :param N : number of samples
        :param m : integer value indicating the multiple of base sub carrier frequency of 1.023e06
        :returns array of samples with saubcarrier frequencies - m*1.023e06
        '''
...
class NavicDataGen():
    """NavicDataGen generates raw samples of data randomly
    ...
    def SymbolsGen(self):
        ...
        toi_data = randint(1,400)   #Generate a random number
        toi_data_string=bin(toi_data)[2:].zfill(9)
        subframe1_data =  np.array([int(bit) for bit in toi_data_string])#Convert it into 9 bits
        subframe1_encoded = self.bch_encode(subframe1_data)
        frame = np.append(frame, subframe1_encoded) # Subframe1 is added to master frame
        nav_data=np.append(nav_data,subframe1_data)
        
        # Subframe 2 - LDPC encoding (1200 symbols)
        subframe2_data = np.array([np.random.randint(0, 2) for _ in range(576)]) # Generate 576 random values [0..1]
        subframe2_crc = rtk_crc24q(subframe2_data, len(subframe2_data)) # Add 24 CRC bits
        ...
        subframe2_encoded = self.ldpc_encode(A_s2,B_s2,C_s2,D_s2,E_s2,T_s2,nav_crc) #LDPC Encoding to generate 1200 symbols from 600 bits
        frame_sub = np.append(frame_sub, subframe2_encoded) # Subframe2 is added to frame_sub array for interleaving purposes
        ...
        subframe3_crc = rtk_crc24q(subframe3_data, len(subframe3_data))  # Add 24 CRC bits
        ...
        subframe3_encoded = self.ldpc_encode(A_s3,B_s3,C_s3,D_s3,E_s3,T_s3,nav_crc)#LDPC Encoding to generate 548 symbols from 274 bits
        frame_sub = np.append(frame_sub, subframe3_encoded) #Subframe3 is added to frame_sub array for interleaving purposes
        nav_data=np.append(nav_data,nav_crc)
        ...
        frame = np.append(frame, nav_intrlv)# combine Subframe1 and interleaved Subframe2 and Subframe3

        return frame, nav_data
    
    #This function generates required submatrices for Subframe2 LDPC encoding. Ref : NavicL1 ICD document
    ...
    #This function generates required submatrices for Subframe3 LDPC encoding. Ref : NavicL1 ICD document
...
def navic_pcps_acquisition(x, prnSeq_pilot, fs, fSearch,coherent_integ, non_coherent_integ, threshold=4):
    ...
    :param coherent_integ : number of coherent integrations to be carried out
    :param non_coherent_integ : number of non coherent integrations to be carried out
    ...
    fDev = 0
    tau = 0
    full = np.empty((K, N), dtype=np.complex_)
    for i in range(0,N):
...
class NavicTracker:
    ...
    def stepImpl(self, u):
       ...
       #The below line is added to fix the bug related to receiveing correct data with one bit delay
        ...
        numSamplesPerHalfChip = round(self.pSamplesPerChip*0.15) # early, late
        numSamplesPerThreeQuarterChip = round(self.pSamplesPerChip*0.6) # very early , very late
