...
class NavicL5sModulator():
    ...
    def Modulate(self, x, codeTable1,codeTable2,codeTable3):
        ...
        #print("boc pilot", BocPilotSig1)
        #print("boc Data", BocDataSig1)
...
def navic_pcps_acquisition(x, prnSeq_pilot, fs, fSearch, threshold=0):
    ...
    #pilotCode = (prnSeq_pilot+prnSeq_pilot_overlay)%2 # pilot and overlay codes are multiplied
    #pilotSig = 1-2*pilotCode
   ...
   # ts = 1/fs
    ...
    maxCol = maxIndex%N
    maxRow = maxIndex//N
   # print("Max value is ", abs(Rxd[maxRow, maxCol])**2)
...
class NavicTracker:
    def __init__(self, prnId, dataCodeLength, pilotCodeLength):
        ...
        self.pilotCodeStart = 0
        self.dataCodeStart = 0
        self.codeTable_data = genNavicCaCode_Data(self.PRNID)
        self.codeTable_pilot = genNavicCaCode_Pilot(self.PRNID)
    ...
    def updatePromptCode(self):
       ...
       # codeNumSample_pilot = self.codeTable_pilot.shape[0]
        ...
        chips_covered = int(self.PLLIntegrationTime*1e-3*self.ChipRate)
        ...
        # Subcarrier generation for BOC
        t = np.arange(int(self.ChipRate*self.PLLIntegrationTime*1e-3))/(int(self.ChipRate*self.PLLIntegrationTime*1e-3))
        subCarrier = np.sign(np.sin(2*np.pi*(self.subcarrierFrequency*t+self.subCarrPhase1)))
        
        # 
        #pilotCode = (self.codeTable_pilot + codeTable_o_rep)%2
        #pilotSig = 1-2*pilotCode
        
        pilotSig = 1-2 * self.codeTable_pilot[self.pilotCodeStart:self.pilotCodeStart+chips_covered]
        
        dataCodeSig = 1-2*self.codeTable_data[self.pilotCodeStart:self.pilotCodeStart+chips_covered]
        
        
        pilot_code = pilotSig*subCarrier # added subcarrier multipication
        data_code = dataCodeSig * subCarrier
        
        self.pilotCodeStart = (self.pilotCodeStart+chips_covered)%self.pilotCodeLength
        self.dataCodeStart = (self.dataCodeStart+chips_covered)%self.dataCodeLength
        ...
        self.numSamplesPerCodeBlock = self.SampleRate * 1e-3 # As each code block is of 10e-3 seconds
        pilot_upwave1 = np.repeat(pilot_code, upSampleFactor)
        self.pPromptCode_pilot = np.tile(pilot_upwave1[::downSampleFactor], numCACodeBlocks)
        
        data_upwave1 = np.repeat(data_code, upSampleFactor)
        self.pPromptCode_data = np.tile(data_upwave1[::downSampleFactor], numCACodeBlocks)
    ...
    def stepImpl(self, u):
       ...
       # [u_data, self.pBuffer_data] = np.split(np.append(self.pBuffer_data, local_u), [round(self.SampleRate*integtime)])
        ...
        #iqsig_data = u* np.exp(-1j*phases[:-1])
        ...
        iq_e = iqsig * np.roll(self.pPromptCode_pilot, -1*numSamplesPerHalfChip) # Early
        iq_ve = iqsig * np.roll(self.pPromptCode_pilot, -1*numSamplesPerThreeQuarterChip)# Very Early
        iq_p = iqsig * self.pPromptCode_pilot # Prompt for Pilot
        iq_l = iqsig * np.roll(self.pPromptCode_pilot, numSamplesPerHalfChip) # Late
        iq_vl = iqsig * np.roll(self.pPromptCode_pilot, numSamplesPerThreeQuarterChip) # Very Late
        
        integeval = np.sum(iq_e)
        integlval = np.sum(iq_l)
        integveval = np.sum(iq_ve)
        integvlval = np.sum(iq_vl)
        ...
        millisecdata_pilot = iq_p.reshape((int(self.PLLIntegrationTime/10), -1)).T # Each column contains ten millisecond of data
        y = np.sum(millisecdata_pilot, axis=0) # Each element contains integrated value of one millisecond of data
        integpval = np.sum(y)
        ...
        # DLL discriminator
        E = np.linalg.norm([integeval, integveval])
        L = np.linalg.norm([integlval, integvlval])
        
        #delayerr = (E-L)/(2*(E+L)) # Non-coherent early minus late normalized detector
        delayerr = (E-L)/(E+L) # Non-coherent early minus late normalized detector
        ...
        if self.DisablePLL:
            ...
            #pherr = np.arctan2(np.real(integpval),np.imag(integpval))/(2*np.pi)
             pherr = np.arctan2(np.imag(integpval),np.real(integpval))/(2*np.pi)
        ...
        return y, y_data, fqyerr, fqynco, pherr, phnco, delayerr, delaynco
...
# If a satellite is visible, initialize tracking loop
            if(status == True):
                satVis += 1 
                tracker.append(NavicTracker(satId[prnId], dataCodeLength, pilotCodeLength))
    ...
    for i in range(satVis):
      y[istep, i], y_data[istep,i],fqyerr[istep, i], fqynco[istep, i], pherr[istep, i], phnco[istep, i], delayerr[istep, i], delaynco[istep, i] = tracker[i].stepImpl(waveform)

print(len(y))

k = len(y)    
mapbit = lambda y: np.piecewise(np.imag(y), [np.imag(y) < 0, np.imag(y) >= 0], [1, 0])
mapbitinv = lambda y: np.piecewise(np.imag(y), [np.imag(y) < 0, np.imag(y) >= 0], [0, 1])

map_pilot_overlay = lambda y: np.piecewise(np.real(y), [np.real(y) < 0, np.real(y) >= 0], [1, 0])
map_pilot_overlay_inv = lambda y: np.piecewise(np.real(y), [np.real(y) < 0, np.real(y) >= 0], [0, 1])
 
#np.set_printoptions(threshold=np.inf)
    
#print(mapbit)

pilot_overlay_sent = data_sent = pilotOverlayCodegen.GetBitStream()[:-1, 0]
y_regular = map_pilot_overlay(y[:, 0])
y_inverted = map_pilot_overlay_inv(y[:, 0])
...
num =int(k/dlen)
count = 0
for outer in range(dlen): 
    for iter in range(num):
    #print("data sent =", data_sent[iter], "data reg=", y_data_regular[iter], "data inverted=", y_data_inverted[iter])
    #print(iter,"pilot sent=", pilot_overlay_sent[iter],"pilot sec code reg=", y_regular[iter], "inverted=", y_inverted[iter], pilot_overlay_sent[iter] == y_regular[iter],y[iter,0])
        print(count,"data sent=", data_sent[outer],"data reg=", y_data_regular[count], "inverted=", y_data_inverted[count],y_data[count,0])
        count = count +1
