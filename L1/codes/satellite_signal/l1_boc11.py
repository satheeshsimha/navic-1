...
def navic_pcps_acquisition(x, prnSeq_pilot, prnSeq_data, fs, fSearch,coherent_integ, non_coherent_integ, K1,if_frequency,threshold=12):
    ...
    fDev = 0
    tau = 0
    subCarrier_boc11 = np.sign(np.sin(2*np.pi*fsc*t_subcarrier+phase1))
    subCarrier_boc61 = np.sign(np.sin(2*np.pi*6*fsc*t_subcarrier+phase2))
                
    BocPilotSig1 = pilotSig * subCarrier_boc11
    BocDataSig1 = dataSig * subCarrier_boc11
    BocPilotsig6 = pilotSig * subCarrier_boc61
    interplexSig = BocPilotSig1* BocDataSig1 *  BocPilotsig6
    prnSeq_pilot_FFT = np.conjugate(np.fft.fft((alpha*BocPilotSig1)+ (1j*(gamma*BocDataSig1))))
    
    for i in range(0,N):
        Rxd = np.zeros(K1)
        non_coherent_prod = np.zeros(K1, dtype=np.complex_)
        #x_iq = x*np.exp(-1j*2*np.pi*(fSearch[i]+if_frequency)*t)
        for  a in range(0,non_coherent_integ):
            coherent_prod = np.zeros(K1, dtype = np.complex_)
            for b in range(0, coherent_integ):
                start = a*K1
                end = (a*K1)+(K1)
                x_iq = x[start:end]*np.exp(-1j*2*np.pi*(fSearch[i]+if_frequency)*t_subcarrier)
                #x_iq = x*np.exp(-1j*2*np.pi*(fSearch[i]+if_frequency)*t_subcarrier)
                XFFT = np.fft.fft(x_iq)
    
                YFFT = XFFT*prnSeq_pilot_FFT
                coherent_prod = coherent_prod+YFFT
                                   
            non_coherent_prod =  (1/K1)*np.fft.ifft(coherent_prod)
            Rxd = Rxd + (np.abs(non_coherent_prod))**2
            
        # #full[:,i]= Rxd_p
        #Rxd = Rxd/non_coherent_integ 
        dummy = np.copy(Rxd)
        ...
        for ind in range(-5,4,1):
            index = (maxIndex + ind+ K1)%K1 # Make values 0 around one chip duration on either side of max value
            Rxd[index] = 0
        noise = (np.sum(Rxd))/(K1-36)
        p2n = max_val/noise   
        if( p2n > max_of_max ) :
            max_of_max = p2n
            fDev = fSearch[i]
            tau = maxIndex
            noise_maxP2n = noise
            maxPower_maxP2n = max_val
        
    print("Pilot peak to noise ratio =", max_of_max,  "max power=", maxPower_maxP2n,"corresponding noise=", noise_maxP2n) 
    
    if(max_of_max > threshold):
        # print ("tau=",tau, "doppler=", fDev)
        # plt.plot(dummy1)
        # plt.xlabel('time') ; plt.ylabel('RX')
        # plt.show()
...
def navic_pcps_acquisition1(x, prnSeq_pilot, prnSeq_data, fs, fSearch,
                           coherent_integ, non_coherent_integ, K1,
                           if_frequency, threshold=12, plot=False):
    ...
    # Subcarrier
    fsc = 1.023e6  
    subCarrier_boc11 = np.sign(np.sin(2*np.pi*fsc*t_subcarrier))
    ...
    # Combine for NavIC L1 SPS
    alpha = (6/11)**0.5
    gamma = (4/11)**0.5
    BocPilotSig1 = pilotSig * subCarrier_boc11
    BocDataSig1  = dataSig * subCarrier_boc11
    prnSeq_pilot_FFT = np.conjugate(
        np.fft.fft((alpha*BocPilotSig1) + (1j*gamma*BocDataSig1))
    )
    
    N = fSearch.shape[0]
    search_space = np.zeros((K1, N))  # store correlation power

    max_of_max = 0
    fDev, tau = 0, 0
    
    for i in range(N):  # Doppler bins
        Rxd = np.zeros(K1)
        for a in range(non_coherent_integ):
            coherent_prod = np.zeros(K1, dtype=np.complex_)
            for b in range(coherent_integ):
                start = a*K1
                end   = (a+1)*K1
                x_iq = x[start:end] * np.exp(-1j*2*np.pi*(fSearch[i]+if_frequency)*t_subcarrier)
                XFFT = np.fft.fft(x_iq)
                ...
                coherent_prod += YFFT
            non_coherent_prod = (1/K1)*np.fft.ifft(coherent_prod)
            Rxd += (np.abs(non_coherent_prod))**2
        
        search_space[:, i] = Rxd  # store full correlation
        ...
        max_val  = Rxd[maxIndex]
        Rxd_ = np.copy(Rxd)
        for ind in range(-5,4):
            idx = (maxIndex + ind + K1) % K1
            Rxd_[idx] = 0
        noise = np.sum(Rxd_)/(K1-9)
        p2n = max_val/noise
        if p2n > max_of_max:
            max_of_max = p2n
            fDev, tau = fSearch[i], maxIndex
    
    maxIdx = np.argmax(search_space)
    maxCol = maxIdx%N
    maxRow = maxIdx//N
    peakVal = search_space[maxRow,maxCol]
    print("Maxcol=",fSearch[maxCol],"maxRow=",maxRow,"maxVal=",search_space[maxRow,maxCol])
    
    print("Best peak-to-noise ratio =", max_of_max)
    
    if max_of_max > threshold:
        #return True, tau, fDev, search_space
        return True, tau, fDev
    else:
        return False, 0, 0
...
class LockIndicator():
    ...
    def CN0_cap(self):
        ...
        if (l == self.buf._capacity):
              ...
              if ((estimated_total_power - estimated_sig_power) != 0):
    ...
    def phase_lock_indicator(self) :
        ...
        if (l == self.buf._capacity):
            ...
            # a = np.sum(np.imag(self.buf))**2
            # b = np.sum(np.real(self.buf))**2
            if ((a+b) != 0 ):
                pli =  (a-b)/(a+b)
            return pli  
        else : 
...
class NavicTracker:
    def __init__(self, prnId ):
        ...
        self.CodePeriod = 10 # In milliseconds
        self.corr_count = 0
        ...
        self.propmptCode_Pilot_CurrentIndex = 0
        self.propmptCode_Data_CurrentIndex = 0
        ...
        self.subCarrPhase1 = 0 
        self.subCarrPhase2 = 0
        self.subCarrier11 = None
        self.subCarrier61 = None
        self.BOC11_prncode_pilot = None
        self.BOC11_prncode_data  = None
        ...
        #Kalaman Filter Settings for Frequency Error estimate
        self.state_estimate_freq = 0.0
        self.state_covariance_freq = 1.0
        self.process_covariance_freq = 20**2 #0.01**2
        self.measurement_covariance_freq = 100*2 #0.1**2
        
        #parameters for fine delay
        self.space_left_in_20msbuf = 0
        self.samples_left_in_temp_buf = 0
        self.curr_len_20msbuf = 0
        self.current_copy_index = 0
        self.remaining_samples = 0
        self.sample_count = 0 
        
        # reset the correlator output
        self.y_pilot = 0 +1j*0
        self.y_data = 0 +1j*0
        self.Correlators = 0
        
        self.subCarrArray = np.tile([1,-1],self.dataCodeLength)
        
        self.early_late_spacing = 0.25
        self.very_early_very_late_spacing = 0.5
        
        

    def circular_block_access(self, arr, start_index, block_size):
        result = []
        array_length = len(arr)
    
        # Iterate through the block
        for i in range(start_index, start_index + block_size):
        # Wrap around the index if it exceeds the array length
    ...
    def setupImpl(self, dataCodeLength, U, cn0_min, detector_threshold, lock_fail_counter_threshold):
        ...
        self.pNumIntegSamples = round(self.SampleRate*self.PLLIntegrationTime*1e-3)
        ...
        if self.DLLOrder == 1: # Table 8.23 in Kaplan 3rd edition [1]
            ...
            self.pDLLGain2 = (self.pDLLNaturalFrequency**2)*1.1
        ...
        if self.FLLOrder == 1: # Table 8.23 in Kaplan 3rd edition [1]
            ...
            self.pFLLGain2 = (self.pFLLNaturalFrequency**2)*1.1
        ...
        if self.PLLOrder == 1: # Table 8.23 in Kaplan 3rd edition [1]
            ...
            self.pPLLGain2 = (self.pPLLNaturalFrequency**2)*1.1
        ...
        self.remcode_pilot = 0
        self.old_remcode_pilot = 0
        self.remcode_data = 0
        self.old_remcode_data = 0
        
        self.remcarr = 0
        self.old_remcarr = 0
        self.currnsamp = self.pNumIntegSamples
        self.codeFreq = self.ChipRate
        self.carrierFrequency = self.InitialDopplerShift + self.CenterFrequency
        self.carrfreq = self.carrierFrequency
        
        #variables related to fine delay correction
        self.space_left_in_20msbuf = 0
        self.samples_left_in_temp_buf = 0
        self.current_copy_index = self.InitialCodePhaseOffset
        self.remaining_samples = 0
        self.samp_20msec_buff = np.array(self.pNumIntegSamples*2, dtype = complex)
        self.temp_buff  = np.array(self.pNumIntegSamples, dtype = complex) 
        self.curr_len_20msbuf = 2 * self.pNumIntegSamples
    ...
    def prnCode_InitialSetup(self):
        indexArray = 0.5*np.arange(2*self.dataCodeLength)
        indexArray = indexArray.astype(int)
        
        prncode_pilot = self.codeTable_pilot[indexArray]
        prncode_data = self.codeTable_data[indexArray]
        
        alpha = (6/11)**0.5
        gamma = (4/11)**0.5
        
        self.BOC11_prncode_pilot = alpha*self.subCarrArray*(1-2*prncode_pilot)
        self.BOC11_prncode_data  = gamma*self.subCarrArray*(1-2*prncode_data)
        
    
    def finedealy_correction(self,samp):
    
        #Copy the samples of length "currn_samp" from starting index "current_copy_index" from 20msec buff to running_samp array*/
        running_samp = np.copy(self.samp_20msec_buff[self.current_copy_index:self.current_copy_index+self.currnsamp])
   
        #If the required samples are not there in 20msec buffer append zeros at that place*/
        if (self.curr_len_20msbuf < self.currnsamp):
            np.append(running_samp, [0]*(self.currnsamp-self.curr_len_20msbuf))
            self.current_copy_index += self.curr_len_20msbuf
        else :
            self.current_copy_index += self.currnsamp
            
        self.remaining_samples = self.curr_len_20msbuf - self.current_copy_index

        # Move remaining samples in 20msec buff  to the front of twenty_ms_buf */
        if (self.remaining_samples > 0):
            self.samp_20msec_buff[:self.remaining_samples] = self.samp_20msec_buff[self.current_copy_index:self.current_copy_index+self.remaining_samples]
        
        #update the parameters
        self.curr_len_20msbuf = self.remaining_samples
        self.current_copy_index = self.remaining_samples
        self.space_left_in_20msbuf = 2 * self.pNumIntegSamples - self.remaining_samples

        if (self.space_left_in_20msbuf > 0) :

            if ((self.samples_left_in_temp_buf > 0) and (self.space_left_in_20msbuf >= self.samples_left_in_temp_buf)) :
        
                self.samp_20msec_buff[self.current_copy_index:self.current_copy_index+self.samples_left_in_temp_buf] = self.temp_buff[self.pNumIntegSamples-self.samples_left_in_temp_buf:]
                self.curr_len_20msbuf += self.samples_left_in_temp_buf
                self.space_left_in_20msbuf -= self.samples_left_in_temp_buf
                self.current_copy_index += self.samples_left_in_temp_buf
                self.samples_left_in_temp_buf = 0
        
            elif ((self.samples_left_in_temp_buf > 0) and (self.space_left_in_20msbuf < self.samples_left_in_temp_buf)) :
                self.samp_20msec_buff[self.current_copy_index:self.current_copy_index+self.space_left_in_20msbuf] = self.temp_buff[self.pNumIntegSamples-self.samples_left_in_temp_buf:self.pNumIntegSamples-self.samples_left_in_temp_buf+self.space_left_in_20msbuf]
                self.samples_left_in_temp_buf -= self.space_left_in_20msbuf
                self.current_copy_index += self.space_left_in_20msbuf
                self.curr_len_20msbuf += self.space_left_in_20msbuf
                self.space_left_in_20msbuf = 0

        if (self.space_left_in_20msbuf > 0):
            #Now we have to fill up the remainig "space_left_in_20msbuf" in 20ms buf
            #read the 10msec of samples from file to temp buff */
            self.temp_buff = np.copy(samp)

            #fill up newly copied 10msec samples into remaining space of 20msbuf
            if (self.space_left_in_20msbuf >= self.pNumIntegSamples):
                self.samp_20msec_buff[self.current_copy_index:self.current_copy_index+self.pNumIntegSamples] = self.temp_buff
                self.space_left_in_20msbuf -= self.pNumIntegSamples
                self.current_copy_index += self.pNumIntegSamples
                self.curr_len_20msbuf += self.pNumIntegSamples
                self.samples_left_in_temp_buf = 0
            else :
                self.samp_20msec_buff[self.current_copy_index:self.current_copy_index+self.space_left_in_20msbuf] = self.temp_buff[:self.space_left_in_20msbuf]
                self.curr_len_20msbuf += self.space_left_in_20msbuf
                self.samples_left_in_temp_buf = self.pNumIntegSamples - self.space_left_in_20msbuf
                self.current_copy_index += self.space_left_in_20msbuf
                self.space_left_in_20msbuf = 0
    ...
    def new_prn_code_interpolation1(self, output_prn_code_pilot, output_prn_code_data,
                                   ci, coff, n, len):

        #make the initial_code_offet in range of (0,len)
        alpha = (6/11)**0.5
        gamma = (4/11)**0.5
        
        #wrap into [0,len]
        coff %= len
        ##Generate sample by sample replica vectors
        samp_idx = np.arange(n, dtype=np.float64)
        a = samp_idx*ci
        
        phase_P    = (coff + a) % len
        
        chip_index_P   = phase_P.astype(int)
        sub_index_P    = (2*phase_P).astype(int)        # 0…2LEN-1
        
        output_prn_code_pilot[:] = alpha * self.subCarrArray[sub_index_P] * (1 - 2 * self.codeTable_pilot[chip_index_P])
        
        output_prn_code_data[:]  = gamma * self.subCarrArray[sub_index_P] * (1 - 2 * self.codeTable_data[chip_index_P])
         
        coff_next = phase_P[-1] + ci - len
            
        return coff_next
    
    
    def stepImpl(self, u, iter):
        # Implement algorithm. Calculate y as a function of input u and
        # discrete states.
        
        np.set_printoptions(precision=5,suppress = True) 
        self.currnsamp = round((self.dataCodeLength - self.remcode_pilot)/(self.codeFreq/self.SampleRate))
        
        #Apply fine delay correction
        local_u = self.finedealy_correction(u)
        
        # Buffer the input
        integtime = self.PLLIntegrationTime*1e-3 # PLLIntegrationTime is in milliseconds. Hence multiply by 1e-3 to get it into sec
        
        #copy the previous integrated values to the old values
        self.old_y_pilot = self.y_pilot
        self.old_y_data = self.y_data
        
        # reset the correlator output
        self.y_pilot = 0 +1j*0
        self.y_data = 0 +1j*0
        
        self.old_remcode_pilot = self.remcode_pilot
        self.old_remcode_data = self.remcode_data
        
        self.old_remcarr = self.remcarr
        
        # Carrier wipe-off
        fc = self.CenterFrequency + self.InitialDopplerShift - self.pFLLNCOOut
        t = np.arange(round(self.currnsamp)+1)/self.SampleRate
        phases = (2*np.pi*fc*t + self.old_remcarr -self.pPLLNCOOut  )
        iqsig = local_u * np.exp(-1j*phases[:-1])
        
        self.remcarr = ((phases[-1]) + self.pPLLNCOOut)%(2*np.pi) 
        
        prn_code_prompt_pilot = np.zeros(self.currnsamp)
        prn_code_prompt_data = np.zeros(self.currnsamp)

        self.remcode_pilot = self.new_prn_code_interpolation1(prn_code_prompt_pilot,prn_code_prompt_data,
                                                              (1/self.SampleRate) * self.codeFreq, 
                                                              self.old_remcode_pilot, self.currnsamp, 
                                                              self.dataCodeLength) 
        numSamples_Early_Late_Chip = round(self.pSamplesPerChip*self.early_late_spacing)
        numSamples_VeryEarly_VeryLate_Chip = round(self.pSamplesPerChip*self.very_early_very_late_spacing )
        prn_code_very_early_pilot = np.roll(prn_code_prompt_pilot, numSamples_VeryEarly_VeryLate_Chip )
        prn_code_early_pilot = np.roll(prn_code_prompt_pilot, numSamples_Early_Late_Chip)
        prn_code_late_pilot = np.roll(prn_code_prompt_pilot, -numSamples_Early_Late_Chip)
        prn_code_very_late_pilot = np.roll(prn_code_prompt_pilot, -numSamples_VeryEarly_VeryLate_Chip)
        
        
        
        iq_ve_pilot = iqsig*prn_code_very_early_pilot
        iq_e_pilot = iqsig*prn_code_early_pilot
        iq_p_pilot = iqsig*prn_code_prompt_pilot
        iq_l_pilot = iqsig*prn_code_late_pilot
        iq_vl_pilot = iqsig*prn_code_very_late_pilot
        ...
        fqyerr = fqyerr_pilot
        #First Oder FLL
        fqynco = self.pFLLNCOOut + integtime*fqyerr*self.pFLLGain1
        ...
        if ( (E_pilot + L_pilot ) != 0) :
                delayerr_pilot = (E_pilot - L_pilot )/((E_pilot + L_pilot )) # Non-coherent early minus late normalized detector
                
        delayerr = delayerr_pilot
        
        #2nd Order DLL
        delaynco = self.pDLLNCOOut + self.pDLLGain2*(delayerr- self.pDLLWPrevious1) + self.pDLLGain1*integtime*delayerr
        self.pDLLNCOOut = delaynco
        
        self.pDLLWPrevious1 = delayerr
        
        #DLL
        self.codeFreq = self.ChipRate - self.pDLLNCOOut
        # Phase lock indictor and SNR Threshold calcuator
        
        cn0_cap, pli = self._Lock_indicator_calc(self.y_data)
        ...
        return self.currnsamp, self.y_pilot, self.y_data, fqyerr, fqynco, pherr, self.pPLLNCOOut, delayerr, self.pDLLNCOOut, cn0_cap, pli, self.lock_fail_counter,self.carrfreq, self.remcode_pilot
    ...
    def __upsample_table(self, codeBase):
        ...
        samplingPeriod = 1/self.SampleRate
        indexArr = (np.arange(self.currnsamp)*samplingPeriod*self.ChipRate).astype(np.float32)     # Avoid floating point error due to high precision
        indexArr = (indexArr.astype(int))%self.dataCodeLength
    ...
    def spectrum_plot (self, Data):
        num_samples = len(Data)
        # Compute FFT
        fft_result = np.fft.fft(Data)
        fft_freqs = np.fft.fftfreq(num_samples, d=1/self.SampleRate)
        
        # Shift the frequencies for correct ordering
        fft_values_shifted = np.fft.fftshift(fft_result)
        freqs_shifted = np.fft.fftshift(fft_freqs)
        
        # Compute Power Spectral Density (in dB)
        psd = 20 * np.log10(np.abs(fft_values_shifted) / num_samples)
        print (psd[np.argmax(psd)], np.argmax(psd))
        # Plot the Spectrum
        plt.figure(figsize=(12, 6))
        plt.plot(freqs_shifted / 1e6, psd, label="Spectrum")  # Convert freq to MHz
        plt.title("Spectrum of NavIC L1 Baseband Signal")
...
def file_read(file, sample_count):
    #F_nums1 = np.fromfile(file, dtype=np.uint8, count=NSAMPLES)
    F_nums11 = np.fromfile(file, dtype=np.uint8, count=sample_count)

    F_nums1 = []
    for i in range(sample_count):
        for j in range(4) : ## make 2 bit data from one byte
            F_nums1.append((F_nums11[i] >> (6-2*j)) & (0b11))
        
    # Convert data as per the byte structure
    F_nums = np.zeros_like(F_nums1, dtype=float)
    for i, val in enumerate(F_nums1):
        if val == 2:
            F_nums[i] = -3
        elif val == 0:
            F_nums[i] = 1
    ...
    # Generate Data
    Data = np.zeros((sample_count // 4, 4), dtype=float)
    for i in range(len(Data)):
        Data[i, 2] = F_nums[4 * i+3]
        Data[i, 3] = F_nums[4 * i + 2]
        Data[i, 0] = F_nums[4 * i + 1]
        Data[i, 1] = F_nums[4 * i + 0]

    data_len = len(Data)*4
    waveform = np.zeros(data_len, dtype=int)
    for i in range(data_len):
        waveform[i] = Data[i // 4, i % 4]
        
    ##Here, number of samples is 185480 instead of 185483
    t_lowerSample_Rate = np.arange(0,1,1/data_len)
    
    # Upsample using interp1d to 185483 
    f_interp = interp1d(t_lowerSample_Rate, waveform, kind='linear', fill_value='extrapolate')
    t_new = np.arange(0, 1, 1/(sample_count))
    return f_interp(t_new)
