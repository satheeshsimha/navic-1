...
#fShift = np.array([4770,4280, 1255, 3755])
fShift = np.array([4780])
...
for istep in range(numSteps):
    ...
    for i in range(satVis):
      y[istep, i], fqyerr[istep, i], fqynco[istep, i], pherr[istep, i], phnco[istep, i], delayerr[istep, i], delaynco[istep, i],fc[istep,i] = tracker[i].stepImpl(waveform)
...
for i in range(satVis):
    ...
    print("sync frames length=",len(sync_frames))
    ...
    for j in range(num_sf):
        print("j=", j)
        print("Equal number of bits=", np.sum(np.equal(check[j], navbits[j,:-6])))
        if(np.array_equal(check[j], navbits[j,:-6])):
            print("check", check[j])
            print("navbits", navbits[j,:-6] )
            print(np.equal(check[j], navbits[j,:-6]))
            print("Equal")
        else:
            print("Not Equal")
    ...
    plt.plot(fc[:,i])
    #plt.ylim([0,0.05])
    plt.xlabel('time') ; plt.ylabel('Fqy ')
